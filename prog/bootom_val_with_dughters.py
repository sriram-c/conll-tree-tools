...
def daughter_bottom_value(tree):
    ...
    dtr = {}
    k = 0
    for i in range(0,len(H_id)):

        if H_id[i] in dtr:
            dtr[H_id[i]].append(A_id[i])
        else:
            dtr[H_id[i]] = [A_id[i]]
    ...
    for i in range(0,len(A_id)):
        if A_id[i] in dtr:
            ...
            for j in dtrs:
                tmp_b_v = b_v[j - 1]
                tmp_lst.append([j,tmp_b_v])
            d_b_v.append(tmp_lst)
        else:
            d_b_v.append(["_"])
...
tree = pd.read_csv(sys.argv[1],sep='\t',header=None)
...
del  tree[8]
del  tree[9]
tree.insert(loc=8, column=8, value=t_v)
tree.insert(loc=9, column=9, value=list_b_val)

d_b_v = daughter_bottom_value(tree)

tree.insert(loc=10, column=10, value=d_b_v)

tree.to_csv(sys.argv[2],sep='\t',header=False,index=False)
